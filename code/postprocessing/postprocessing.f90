...
contains
  ...
  subroutine after_timestep(this, p)
    ...
    integer :: i, j, k, imeasure
    class(t_variable), pointer :: measure

    dbghead(postprocessor :: after_timestep)

    call this%get_index(p%lon0, p%lat0, p%depth0, i, j, k)
  ...
  !===========================================
  integer function get_depth_index(this, dep) result(res)
    ...
    do i = 1, size(this%bin_dep) - 1
      if (this%bin_dep(i) <= dep) then
        res = i
  ...
  !===========================================
  subroutine get_index(this, lon, lat, dep, i, j, k)
    class(t_postprocessor), intent(in) :: this
    real(rk), intent(in) :: lon, lat, dep
