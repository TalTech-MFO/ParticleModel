...
contains
  ...
  subroutine run_counter_snapshot(this, p, i, j, k)
    ...
    integer :: n
    real(rk), dimension(size(this%data, 1), size(this%data, 2), size(this%data, 3)) :: tmp

    tmp = ZERO

    !!!$omp parallel do reduction(+:tmp) schedule(guided)
    do n = 1, size(p)
      if (p(n)%state /= ST_SUSPENDED) cycle
      tmp(i(n), j(n), k(n)) = tmp(i(n), j(n), k(n)) + ONE
    end do
    !!!$omp end parallel do
  ...
  subroutine run_settled_counter_snapshot(this, p, i, j, k)
    ...
    !!!$omp parallel do reduction(+:tmp) schedule(guided)
    do n = 1, size(p)
      if (p(n)%state /= ST_BOTTOM) cycle
      tmp(i(n), j(n), 1) = tmp(i(n), j(n), 1) + ONE
    end do
    !!!$omp end parallel do
  ...
  subroutine run_property_snapshot(this, p, i, j, k)
    ...
    do n = 1, size(p)
      if (p(n)%state /= ST_SUSPENDED) cycle
      value(n) = p(n)%get_variable(this%variable_name)
    end do
    !$omp end parallel do
    ...
    do n = 1, size(p)
      if (p(n)%state /= ST_SUSPENDED) cycle
      tmp(i(n), j(n), k(n)) = tmp(i(n), j(n), k(n)) + value(n)
    end do
    !!!$omp end parallel do
  ...
  subroutine run_settled_property_snapshot(this, p, i, j, k)
    ...
    do n = 1, size(p)
      if (p(n)%state /= ST_BOTTOM) cycle
      tmp(i(n), j(n), 1) = tmp(i(n), j(n), 1) + value(n)
    end do
    !!!$omp end parallel do
