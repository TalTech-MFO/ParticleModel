    ...
    def __init__(self, fname, zipname=None):
        ...
        if self.zipname is not None:
            ...
            if self.get_file_size() > self.SIZE_LIMIT:
                ...
                self.f = Dataset("dummy", memory=self.zf.open(self.fname).read())
        else:
            self.f = Dataset(self.fname)
    ...
    def get_time(self, vname="time"):
        ...
        return np.array(
            [
                ...
                for val in num2date(timein, timeunits)
            ]
        )
...
def file_loop(itime):
    ...
    # Get histogram
    counts = get_counts_by_state(xi, yi, statei, bin_lon, bin_lat).astype(float)

    if scale_dict is not None:
        concentration = get_scaled_concentration(
            xi, yi, statei, ids, bin_lon, bin_lat
        ).astype(float)
    else:
    ...
    # Write to netcdf
    out_dict = {
        f"counts_{state_vals[i]}": counts[i - 1, :, :] for i in state_vals.keys()
    }
    out_dict["counts_sum"] = np.sum(counts, axis=0)
    out_dict = dict(
        out_dict,
        **{
            f"concentration_{state_vals[i]}": concentration[i - 1, :, :]
            for i in state_vals.keys()
        },
    )

    time_dict = {"time": timei, "time_units": "seconds since 1970-01-01 00:00:00"}

    loc_dict = {f"counts{i}": s for i, s in state_vals.items()}
    loc_dict["name"] = "state_vals"
    save_counts(
        fnameout, bin_lon[:-1], bin_lat[:-1], time_dict, loc_dict=loc_dict, **out_dict
    )
    print(f"Saved {fnameout}")


state_vals = {1: "BEACHED", 2: "ON_BOUNDARY", 3: "ACTIVE", 4: "BOTTOM"}
N_STATES = 4
...
def main():
    ...
    parser.add_argument(
        "-s",
        "--source",
        help="Path to data file. If <source> is contained in .zip folder, the path is inside to .zip",
        type=str,
        required=True,
    )
    parser.add_argument(
        "--zip-file",
        help="Path to zipped data file if <source> is contained in .zip folder",
        type=str,
        required=False,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--id-scale",
        help="Dictionary of scale factors for each particle id",
        type=str,
        required=False,
    )
    ...
    topo[topo < -5.0] = np.nan

    m2deg = 1.0 / (1852.0 * 60.0)
    deg2m = 1852.0 * 60.0

    if resolution is not None:
        dlon_bin = resolution * m2deg / np.cos(np.nanmean(lat) * np.pi / 180.0)
        bin_lon = np.arange(np.nanmin(lon), np.nanmax(lon) + dlon_bin, dlon_bin)
        dlat_bin = resolution * m2deg
        bin_lat = np.arange(np.nanmin(lat), np.nanmax(lat) + dlat_bin, dlat_bin)
    else:
        bin_lon = lon
        bin_lat = lat
    ...
    dx = dLon * deg2m * np.cos(np.nanmean(lat) * np.pi / 180.0)
    dy = dLat * deg2m
    ...
    files_to_merge = (
        glob(f"{tmpdir}/*.nc")
        if resolution is None
        else glob(f"{tmpdir}/*.res{int(resolution)}.nc")
    )
    merge_files(files_to_merge, outfname=final_out, overwrite=overwrite)
